...
class SystemUploadFileService extends AbstractService
{
    ...
    public function responseFile(string $hash) : \Psr\Http\Message\ResponseInterface
    {
        ...
        return container()->get(MineResponse::class)->responseImage($this->mineUpload->getFileSystem()->read($this->mineUpload->getStorageMode() === '1' ? str_replace(env('UPLOAD_PATH', 'uploadfile'), '', $model->url) : $model->url), $model->mime_type);
    }
}