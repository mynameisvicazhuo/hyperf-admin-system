...
class SystemQueueLogService extends AbstractService implements QueueLogServiceInterface
{
    ...
    public function addQueue(AmqpQueueVo $amqpQueueVo) : bool
    {
        ...
        return $this->producer->produce(new $class($amqpQueueVo->getData()), $amqpQueueVo->getIsConfirm(), $amqpQueueVo->getTimeout(), $amqpQueueVo->getDelayTime());
    }
    /**
     * 推送消息到队列
    ...
    public function pushMessage(QueueMessageVo $message, array $receiveUsers = []) : bool
    {
        ...
        $data = ['title' => $message->getTitle(), 'content' => $message->getContent(), 'content_type' => $message->getContentType(), 'send_by' => $message->getSendBy() ?: user()->getId(), 'receive_users' => $receiveUsers];
        return $this->producer->produce(new MessageProducer($data), $message->getIsConfirm(), $message->getTimeout(), $message->getDelayTime());
    }
}