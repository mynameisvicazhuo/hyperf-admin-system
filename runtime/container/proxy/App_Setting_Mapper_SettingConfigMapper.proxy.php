...
class SettingConfigMapper extends AbstractMapper
{
    ...
    {
        $model = $this->model::query()->find($key, ['group_id', 'name', 'key', 'value', 'sort', 'input_type', 'config_select_data']);
    ...
    public function getConfigByGroupKey(string $groupKey) : array
    {
        ...
        return $this->model::query()->whereRaw(sprintf('group_id = ( SELECT id FROM %ssetting_config_group WHERE code = ? )', $prefix), [$groupKey])->get()->toArray();
    }
    /**
     * 更新配置
     * @param string $key
    ...
    {
        return $this->model::query()->where('key', $key)->update(['value' => is_array($value) ? json_encode($value, JSON_UNESCAPED_UNICODE) : $value]) > 0;
    }
    /**
     * 保存配置
     * @param array $data
