...
class MineUpload
{
    ...
    protected function handleUpload(UploadedFile $uploadedFile, array $config) : array
    {
        ...
        $fileInfo = ['storage_mode' => $this->getStorageMode(), 'origin_name' => $uploadedFile->getClientFilename(), 'object_name' => $filename, 'mime_type' => $uploadedFile->getClientMediaType(), 'storage_path' => $path, 'hash' => md5_file($tmpFile), 'suffix' => Str::lower($uploadedFile->getExtension()), 'size_byte' => $uploadedFile->getSize(), 'size_info' => format_size($uploadedFile->getSize() * 1024), 'url' => $this->assembleUrl($config['path'] ?? null, $filename)];
    ...
    public function handleChunkUpload(array $data) : array
    {
        ...
        if ($data['index'] === $data['total']) {
            ...
            $fileInfo = ['storage_mode' => $this->getStorageMode(), 'origin_name' => $data['name'], 'object_name' => $fileName, 'mime_type' => $data['type'], 'storage_path' => $storagePath, 'hash' => $data['hash'], 'suffix' => $data['ext'], 'size_byte' => $data['size'], 'size_info' => format_size((int) $data['size'] * 1024), 'url' => $this->assembleUrl(null, $fileName)];
            $this->evDispatcher->dispatch(new \Mine\Event\UploadAfter($fileInfo));
            return $fileInfo;
        }
    ...
    public function handleSaveNetworkImage(array $data) : array
    {
        ...
        $fileInfo = ['storage_mode' => $this->getStorageMode(), 'origin_name' => md5((string) time()) . '.jpg', 'object_name' => $filename, 'mime_type' => 'image/jpg', 'storage_path' => $path, 'suffix' => 'jpg', 'hash' => $hash, 'size_byte' => $size, 'size_info' => format_size($size * 1024), 'url' => $this->assembleUrl($data['path'] ?? null, $filename)];
        $this->evDispatcher->dispatch(new \Mine\Event\UploadAfter($fileInfo));
        return $fileInfo;
    }
    /**
