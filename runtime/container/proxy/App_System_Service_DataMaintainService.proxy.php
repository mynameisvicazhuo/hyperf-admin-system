    ...
    public function getColumnList(mixed $table) : array
    {
        if ($table) {
            ...
            foreach (Db::select($sql, [env('DB_DATABASE'), $table]) as $column) {
                $columnList[] = ['column_key' => $column->COLUMN_KEY, 'column_name' => $column->COLUMN_NAME, 'data_type' => $column->DATA_TYPE, 'column_comment' => $column->COLUMN_COMMENT, 'extra' => $column->EXTRA, 'column_type' => $column->COLUMN_TYPE, 'is_nullable' => $column->IS_NULLABLE];
            }
            return $columnList;
        } else {
            return [];
