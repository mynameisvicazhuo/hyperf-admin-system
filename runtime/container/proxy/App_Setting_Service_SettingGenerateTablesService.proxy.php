...
class SettingGenerateTablesService extends AbstractService
{
     ...
     */
    public function __construct(SettingGenerateTablesMapper $mapper, DataMaintainService $dataMaintainService, SettingGenerateColumnsService $settingGenerateColumnsService, ModuleService $moduleService, ContainerInterface $container)
    {
    ...
    public function loadTable(array $params) : bool
    {
        ...
        try {
            ...
            foreach ($params['names'] as $item) {
                $tableInfo = ['table_name' => $item['name'], 'table_comment' => $item['comment'], 'menu_name' => $item['comment'], 'type' => 'single'];
    ...
    public function sync(int $id) : bool
    {
        ...
        return self::__proxyCall(__CLASS__, __FUNCTION__, self::__getParamsMap(__CLASS__, __FUNCTION__, func_get_args()), function (int $id) use($__function__, $__method__) {
            $table = $this->read($id);
            $columns = $this->dataMaintainService->getColumnList(str_replace(env('DB_PREFIX'), '', $table['table_name']));
    ...
    protected function generateCodeFile(int $id, int $adminId) : SettingGenerateTables
    {
        ...
        $model = $this->read($id);
        $classList = [ControllerGenerator::class, ModelGenerator::class, ServiceGenerator::class, MapperGenerator::class, RequestGenerator::class, ApiGenerator::class, VueIndexGenerator::class, SqlGenerator::class, DtoGenerator::class];
    ...
    protected function packageCodeFile() : string
    {
        ...
        foreach ($files as $file) {
            $archive->addFile($path . '/' . $file->getFilename(), $file->getFilename());
        }
    ...
    protected function addZipFile(\ZipArchive $archive, string $path) : void
    {
        ...
        foreach ($fs->directories($path) as $directory) {
            if ($fs->isDirectory($directory)) {
                ...
                foreach ($files as $file) {
                    $archive->addFile($directory . '/' . $file->getFilename(), str_replace(BASE_PATH . '/runtime/generate/', '', $directory) . '/' . $file->getFilename());
                }
        ...
        foreach ($this->moduleService->getModuleCache() as $item) {
            if ($item['enabled']) {
                ...
                foreach (glob($path) as $file) {
                    $models[] = sprintf('\\App\\%s\\Model\\%s', $item['name'], str_replace('.php', '', basename($file)));
                }
            }
        }
        return $models;
    ...
    public function preview(int $id) : array
    {
        /** @var SettingGenerateTables $model */
        $model = $this->read($id);
        return [['tab_name' => 'Controller.php', 'name' => 'controller', 'code' => make(ControllerGenerator::class)->setGenInfo($model)->preview(), 'lang' => 'php'], ['tab_name' => 'Model.php', 'name' => 'model', 'code' => make(ModelGenerator::class)->setGenInfo($model)->preview(), 'lang' => 'php'], ['tab_name' => 'Service.php', 'name' => 'service', 'code' => make(ServiceGenerator::class)->setGenInfo($model)->preview(), 'lang' => 'php'], ['tab_name' => 'Mapper.php', 'name' => 'mapper', 'code' => make(MapperGenerator::class)->setGenInfo($model)->preview(), 'lang' => 'php'], ['tab_name' => 'Request.php', 'name' => 'request', 'code' => make(RequestGenerator::class)->setGenInfo($model)->preview(), 'lang' => 'php'], ['tab_name' => 'Dto.php', 'name' => 'dto', 'code' => make(DtoGenerator::class)->setGenInfo($model)->preview(), 'lang' => 'php'], ['tab_name' => 'Api.js', 'name' => 'api', 'code' => make(ApiGenerator::class)->setGenInfo($model)->preview(), 'lang' => 'javascript'], ['tab_name' => 'Index.vue', 'name' => 'index', 'code' => make(VueIndexGenerator::class)->setGenInfo($model)->preview(), 'lang' => 'html'], ['tab_name' => 'Menu.sql', 'name' => 'sql', 'code' => make(SqlGenerator::class)->setGenInfo($model, user()->getId())->preview(), 'lang' => 'mysql']];
    }
}