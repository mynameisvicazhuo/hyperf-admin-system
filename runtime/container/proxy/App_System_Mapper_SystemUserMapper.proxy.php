...
class SystemUserMapper extends AbstractMapper
{
    ...
    public function handleSearch(Builder $query, array $params) : Builder
    {
        if (!empty($params['dept_id']) && is_string($params['dept_id'])) {
            ...
            $query->selectRaw(Db::raw("DISTINCT {$tablePrefix}system_user.*"))->join('system_user_dept as dept', 'system_user.id', '=', 'dept.user_id')->whereIn('dept.dept_id', SystemDept::query()->where(function ($query) use($params) {
                $query->where('id', '=', $params['dept_id'])->orWhere('level', 'like', $params['dept_id'] . ',%')->orWhere('level', 'like', '%,' . $params['dept_id'])->orWhere('level', 'like', '%,' . $params['dept_id'] . ',%');
            })->pluck('id')->toArray());
        }
        ...
        if (!empty($params['role_id'])) {
            $tablePrefix = env('DB_PREFIX');
            $query->whereRaw("id IN ( SELECT user_id FROM {$tablePrefix}system_user_role WHERE role_id = ? )", [$params['role_id']]);
        }
        if (!empty($params['post_id'])) {
            $tablePrefix = env('DB_PREFIX');
            $query->whereRaw("id IN ( SELECT user_id FROM {$tablePrefix}system_user_post WHERE post_id = ? )", [$params['post_id']]);
        }
        return $query;
    }
    /**
