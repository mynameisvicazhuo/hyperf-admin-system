...
class SystemAppService extends AbstractService
{
    ...
    public function bind(int $id, array $ids) : bool
    {
        ...
        return self::__proxyCall(__CLASS__, __FUNCTION__, self::__getParamsMap(__CLASS__, __FUNCTION__, func_get_args()), function (int $id, array $ids) use($__function__, $__method__) {
    ...
    public function getSignature($appSecret, $params) : string
    {
        ...
        $data = ['sign_ver' => ApiController::SIGN_VERSION, 'app_secret' => $appSecret];
    ...
    public function verifyNormalMode(string $accessToken, array &$apiData) : int
    {
        ...
        if (isset($apiData['app_id'])) {
            ...
            if (!$this->checkAppHasBindApi((int) $appInfo['id'], (int) $apiData['id'])) {
                return MineCode::API_UNBIND_APP;
            }
        }
        return MineCode::API_VERIFY_PASS;
    ...
    {
        return Db::table('system_app_api')->where('app_id', $appId)->where('api_id', $apiId)->count() > 0;
    }
}